...
for t in ${job_list[@]}; do
  ...
  model_dir="../analysis-result/$application/$application-$version/models"
  echo "Task#$counter is frame level $frame of issue $case. This crash happened in $application version $version. task configurations -> execution_idx: $execution_idx, search budget: $search_budget, p_object_pool: "$p_object_pool", seed_clone: $seed_clone, seed_mutations: $seed_mutations"

  ## Start the search process:
  java -d64 -Xmx4000m -jar ../lib/botsing-reproduction.jar -project_cp "../bins/$application/$version/bin/" -crash_log "../crashes/$application/$case/$case.log" -target_frame $frame -Dsearch_budget=$search_budget -Dreset_static_fields=FALSE -Dvirtual_fs=TRUE -Drandom_tests=0 -Dmax_recursion=50 -Dvirtual_net=FALSE -Dreplace_calls=FALSE -Duse_separate_classloader=FALSE -Dstopping_condition=MAXFITNESSEVALUATIONS -model $model_dir -Dcarve_object_pool=FALSE -Dp_object_pool=$p_object_pool -Dseed_clone=$seed_clone -Dseed_mutations=$seed_mutations -Dtest_dir="results/$case-$frame-$seed_clone-$p_object_pool-$execution_idx" > "logs/$case-$frame-$seed_clone-$p_object_pool-$execution_idx-out.txt" 2> "logs/$case-$frame-$seed_clone-$p_object_pool-$execution_idx-err.txt" &
  pid=$!
  echo $pid
